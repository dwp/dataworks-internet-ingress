module "vpc" {
  ...
  interface_vpce_source_security_group_ids = concat([aws_security_group.reverse_proxy_ecs[0].id], local.ssh_bastion_enabled[local.environment] ? [aws_security_group.ssh_bastion[0].id] : [])
